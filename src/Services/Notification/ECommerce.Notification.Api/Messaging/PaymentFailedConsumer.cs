...
public sealed class PaymentFailedConsumer : IConsumer<PaymentFailed>
{
    ...
    public Task Consume(ConsumeContext<PaymentFailed> context)
    {
        ...
        return notificationService.CreateAsync(new CreateNotificationRequest(context.Message.CustomerId, context.Message.OrderId, "payment.failed", title, message, "en"), context.CancellationToken);
    }
}
