...
public sealed class PaymentCommandHandlerTests
{
    ...
    public async Task AuthorizePersistsProviderReferences()
    {
        ...
        AuthorizePaymentCommandHandler handler = new(
            new PaymentAuthorizationService(repository, repository, repository, provider));
    ...
    public async Task RefundPersistsProviderTransactionReference()
    {
        ...
        InMemoryPaymentRepository repository = new();
        StubPaymentProvider provider = new();
        PaymentAuthorizationService authorizationService = new(
            repository, repository, repository, provider);
        PaymentRefundService refundService = new(repository, repository, repository, provider);
        AuthorizePaymentCommandHandler authorizeHandler = new(authorizationService);
        RefundPaymentCommandHandler refundHandler = new(refundService);
