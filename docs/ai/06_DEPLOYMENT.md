...
- structure: `WorkflowCheckRunner` registers the isolated user and selects scenario strategies; each `IWorkflowScenarioCheck` implementation owns one scenario, while `WorkflowScenarioContext` centralizes order creation and bounded polling. Adding a scenario does not extend a central switch statement.
...
- role-sync preflight: `scripts/test-auth0-management-access.ps1` validates all Identity Management settings and obtains a disposable token scoped exactly to `update:users`. It performs no user or role mutation and prevents a misconfigured or over-scoped role-sync client from reaching application startup.
